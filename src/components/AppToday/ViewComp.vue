<script setup>
let groupArr = ['Default', 'Due date', 'Assignee', 'Date added', 'Priority', 'Label', 'Project'];
let sortArr = ['Default', 'Alphabetically', 'Assignee', 'Due date', 'Date added', 'Priority', 'Project']
let assigne = ['Default', 'No one','Only me'];
...
<template>
    <div
        v-show="store.viewComp" 
        class="w-1/5 border md:mt-2 h-auto z-30 absolute md:right-20 rounded md:p-2">
        <p class="font-bold md:pl-2">Sort</p>
        <div class="flex items-center rounded md:p-1 justify-between hover:bg-gray-200 group">
            <div class="text-gray-500 flex items-center">
                ...
                <p class="text-sm">Group by</p>
            </div>
            <select class="select text-gray-500 w-20 text-sm">
                <option v-for="item in groupArr" :key="item" :value="item">{{ item }}</option>
            </select>
        </div>
        <div class="flex md:mt-1 items-center rounded md:p-1 justify-between hover:bg-gray-200 group">
            <div class="text-gray-500 flex items-center">
            ...
            <select class="select text-gray-500 w-20 text-sm">
                <option v-for="item in sortArr" :key="item" :value="item">{{ item }}</option>
            </select>
        </div>
        <div class="flex md:mt-1 items-center rounded md:p-1 justify-between hover:bg-gray-200 group">
            <div class="text-gray-500 flex items-center">
            ...
            <select class="select text-gray-500 w-20 text-sm">
                <option v-for="item in assigne" :key="item" :value="item">{{ item }}</option>
            </select>
        </div>
    </div>
</template>
